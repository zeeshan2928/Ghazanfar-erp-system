...
async function testWorkflow() {
  ...
  if (!billResponse.ok) {
    console.error('❌ Bill creation failed:', billResponse.status, await billResponse.text());
    return;
  }
  ...
  if (!confirmResponse.ok) {
    console.error('❌ Gate pass confirm failed:', confirmResponse.status, await confirmResponse.text());
    return;
  }
  ...
  const gatePass2Response = await fetch(
    'http://localhost:3000/gate-passes?warehouseId=1&skip=0&take=5',
    {
      headers: { 'Authorization': `Bearer ${token}` },
    },
  );
