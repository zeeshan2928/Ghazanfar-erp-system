...
export function PurchaseAnalysisScreen() {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const summary = await apiClient.uploadPurchaseAnalysisReport(formData);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.uploadCard}>
        <h4 style={styles.chartTitle}>Upload a purchase/vendor report (CSV, XLS or XLSX)</h4>
        <input
          type="file"
          ref={fileInputRef}
          accept=".csv,.xls,.xlsx"
        ...
        {lastSummary && (
          <div style={styles.summaryBox}>
            ...
            {lastSummary.parserWarnings.length > 0 && (
              <div style={styles.warningBanner}>
                {lastSummary.parserWarnings.length} parsing note(s), e.g.: {lastSummary.parserWarnings[0]}
              </div>
            )}
